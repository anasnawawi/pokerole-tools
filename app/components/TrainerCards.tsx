"use client";
/* The read-only "card" view of a trainer and their party — split out of what
   used to be the standalone /trainer-card page (now retired: see
   app/trainer-card/page.tsx) so the Characters page can render the same
   cards directly above its editable forms, instead of sending the GM to a
   second page for a glance at the same trainer. */
...
import { BattleLite } from "../lib/session";
...
export function TrainerCard({ trainer, owned }: {
  trainer: TrainerData; owned: number;
}) {
...
export function PokemonCard({ sheetKey, sheet, battle }: {
