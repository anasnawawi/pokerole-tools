...
function AddToInventoryModal({item, onClose}:{item:ItemData; onClose:()=>void}) {
  ...
  if(trainers.length===0) return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#1e2235",border:"1px solid #3a4060",borderRadius:10,padding:24,maxWidth:360}} onClick={e=>e.stopPropagation()}>
        <p style={{color:"#8b90a8",fontSize:13}}>No characters saved. Create trainers first.</p>
        <button onClick={onClose} style={{marginTop:12,padding:"6px 14px",borderRadius:5,border:"none",background:"#3a4060",color:"#e8eaf0",cursor:"pointer"}}>Close</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#1e2235",border:"1px solid #3a4060",borderRadius:10,width:380,overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        {/* Header */}
        <div style={{padding:"12px 16px",borderBottom:"1px solid #2a2f45",display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:16}}>🎒</span>
          <div style={{flex:1}}>
            <div style={{fontSize:15,fontWeight:700,color:"#e8eaf0",fontFamily:"'Exo 2'"}}>{item.name}</div>
            <div style={{fontSize:10,color:"#5a6080"}}>{item.category}{item.pocket?` · ${item.pocket}`:""}</div>
          </div>
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
          {/* Trainer */}
          <div>
            <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5}}>Trainer</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {trainers.map((t:any)=>(
                <button key={t.id} onClick={()=>setSelTrainerId(t.id)}
                  style={{padding:"5px 12px",borderRadius:5,fontSize:12,fontWeight:600,cursor:"pointer",border:`1px solid ${selTrainerId===t.id?"#00d4aa":"#3a4060"}`,background:selTrainerId===t.id?"rgba(0,212,170,0.12)":"transparent",color:selTrainerId===t.id?"#00d4aa":"#8b90a8"}}>
                  {t.name}
                  {mode==="purchase"&&<span style={{fontSize:9,color:"#ffd32a",marginLeft:6}}>₽{t.money??0}</span>}
                </button>
              ))}
            </div>
          </div>
          {/* Quantity */}
          <div>
            <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5}}>Quantity</div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={()=>setQty(q=>Math.max(1,q-1))} style={{width:28,height:28,borderRadius:4,border:"1px solid #3a4060",background:"#13151f",color:"#00d4aa",cursor:"pointer",fontSize:16}}>−</button>
              <input type="number" min={1} value={qty} onChange={e=>setQty(Math.max(1,parseInt(e.target.value)||1))}
                style={{width:56,textAlign:"center",background:"#13151f",border:"1px solid #3a4060",borderRadius:4,color:"#e8eaf0",fontSize:14,padding:"4px 0"}}/>
              <button onClick={()=>setQty(q=>q+1)} style={{width:28,height:28,borderRadius:4,border:"1px solid #3a4060",background:"#13151f",color:"#00d4aa",cursor:"pointer",fontSize:16}}>+</button>
            </div>
          </div>
          {/* Mode */}
          <div>
            <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5}}>Transaction Type</div>
            <div style={{display:"flex",gap:6}}>
              <button onClick={()=>setMode("gift")} style={{flex:1,padding:"6px 0",borderRadius:5,fontSize:12,fontWeight:600,cursor:"pointer",border:`1px solid ${mode==="gift"?"#00d4aa":"#3a4060"}`,background:mode==="gift"?"rgba(0,212,170,0.12)":"transparent",color:mode==="gift"?"#00d4aa":"#8b90a8"}}>🎁 Gifted</button>
              <button onClick={()=>setMode("purchase")} style={{flex:1,padding:"6px 0",borderRadius:5,fontSize:12,fontWeight:600,cursor:"pointer",border:`1px solid ${mode==="purchase"?"#ffd32a":"#3a4060"}`,background:mode==="purchase"?"rgba(255,211,42,0.1)":"transparent",color:mode==="purchase"?"#ffd32a":"#8b90a8"}}>💰 Purchase</button>
            </div>
          </div>
          {/* Price (purchase only) */}
          {mode==="purchase"&&(
            <div>
              <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5}}>Price per item (₽)</div>
              <input type="number" min={0} value={price} onChange={e=>setPrice(Math.max(0,parseInt(e.target.value)||0))}
                style={{width:"100%",background:"#13151f",border:`1px solid ${canAfford?"#ffd32a50":"#ff475750"}`,borderRadius:5,color:"#ffd32a",fontSize:16,padding:"6px 10px",fontFamily:"'Exo 2'",fontWeight:700}}/>
              <div style={{marginTop:6,fontSize:11,color:canAfford?"#5a6080":"#ff4757"}}>
          ...
          <button onClick={confirm} disabled={mode==="purchase"&&!canAfford}
            style={{padding:"8px 0",borderRadius:6,fontSize:13,fontWeight:700,cursor:mode==="purchase"&&!canAfford?"not-allowed":"pointer",border:"none",background:mode==="purchase"&&!canAfford?"#2a2f45":mode==="purchase"?"rgba(255,211,42,0.2)":"rgba(0,212,170,0.2)",color:mode==="purchase"&&!canAfford?"#5a6080":mode==="purchase"?"#ffd32a":"#00d4aa"}}>
...
function ReferenceTabs() {
  ...
  const CAT_ITEM_COLORS: Record<string,string> = {
    MegaStone:"#a040a0", ZCrystal:"#00d4aa", BattleItem:"#f08030",
    Berry:"#78c850", Status:"#f85888", Vitamin:"#6890f0",
    TypeBoosting:"#ffd32a", Tera:"#e04040", "":"#5a6080",
  };
          ...
          <div style={{width:260,display:"flex",flexDirection:"column",background:"#13151f",borderRight:"1px solid #2a2f45",flexShrink:0}}>
              ...
              {tab==="pokedex"&&(
                <div style={{display:"flex",gap:2,flexWrap:"wrap"}}>
                  {(["Starter","Rookie","Standard","Advanced","Expert","Ace","Master","Champion"] as Rank[]).map(r=>(
                    <button key={r} onClick={()=>setSearch(search===r?"":r)} style={{fontSize:8,padding:"2px 5px",borderRadius:3,cursor:"pointer",fontWeight:700,border:`1px solid ${RANK_COLORS[r]}50`,background:search===r?`${RANK_COLORS[r]}25`:"transparent",color:search===r?RANK_COLORS[r]:"#5a6080"}}>{r}</button>
                  ))}
                </div>
              )}
              {tab==="moves"&&(
                <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
                  {(["Physical","Special","Support"] as MoveCategory[]).map(c=>(
                    <button key={c} onClick={()=>setCatFilter(catFilter===c?null:c)} style={{fontSize:9,padding:"3px 6px",borderRadius:3,cursor:"pointer",fontWeight:700,border:`1px solid ${CAT_COLORS[c].text}50`,background:catFilter===c?CAT_COLORS[c].bg:"transparent",color:catFilter===c?CAT_COLORS[c].text:"#5a6080"}}>{c}</button>
                  ))}
                  <button onClick={()=>setSearch(search==="priority:true"?"":"priority:true")} style={{fontSize:9,padding:"3px 6px",borderRadius:3,cursor:"pointer",fontWeight:700,border:"1px solid #00d4aa50",background:search==="priority:true"?"rgba(0,212,170,0.2)":"transparent",color:search==="priority:true"?"#00d4aa":"#5a6080"}}>⚡ Priority Only</button>
                </div>
              )}
            </div>
            <div style={{flex:1,overflowY:"auto",padding:4}}>
              ...
              {tab==="items"&&(
                <>
                  {/* Pocket filters */}
                  <div style={{padding:"6px 4px 4px",borderBottom:"1px solid #2a2f45",marginBottom:4}}>
                    <div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5,paddingLeft:4}}>Pocket</div>
                    <div style={{display:"flex",flexDirection:"column",gap:2}}>
                      <button onClick={()=>{setItemPocketFilter(null);setItemCatFilter(null);}} style={{textAlign:"left",padding:"4px 8px",borderRadius:4,fontSize:11,fontWeight:600,cursor:"pointer",border:"none",background:itemPocketFilter===null?"rgba(0,212,170,0.15)":"transparent",color:itemPocketFilter===null?"#00d4aa":"#8b90a8"}}>🗂 All Pockets</button>
                      {itemPockets.map(p=>{const cfg=POCKET_LABELS[p]||{label:p,color:"#5a6080",icon:"📦"};return(
                        <button key={p} onClick={()=>{setItemPocketFilter(itemPocketFilter===p?null:p);setItemCatFilter(null);}} style={{textAlign:"left",padding:"4px 8px",borderRadius:4,fontSize:11,fontWeight:600,cursor:"pointer",border:"none",background:itemPocketFilter===p?`${cfg.color}20`:"transparent",color:itemPocketFilter===p?cfg.color:"#8b90a8"}}>
                          {cfg.icon} {cfg.label}
                        </button>
                      );})}
                    </div>
                  ...
                  {itemCategories.length>0&&(
                    <div style={{padding:"6px 4px 4px",borderBottom:"1px solid #2a2f45",marginBottom:4}}>
                      <div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:5,paddingLeft:4}}>Category</div>
                      <div style={{display:"flex",flexDirection:"column",gap:2}}>
                        <button onClick={()=>setItemCatFilter(null)} style={{textAlign:"left",padding:"3px 8px",borderRadius:4,fontSize:10,cursor:"pointer",border:"none",background:itemCatFilter===null?"rgba(255,255,255,0.06)":"transparent",color:itemCatFilter===null?"#e8eaf0":"#5a6080"}}>All Categories</button>
                        {itemCategories.map(c=>{const color=CAT_ITEM_COLORS[c]||"#5a6080";return(
                          <button key={c} onClick={()=>setItemCatFilter(itemCatFilter===c?null:c)} style={{textAlign:"left",padding:"3px 8px",borderRadius:4,fontSize:10,cursor:"pointer",border:"none",background:itemCatFilter===c?`${color}20`:"transparent",color:itemCatFilter===c?color:"#5a6080",fontWeight:itemCatFilter===c?700:400}}>
                            {c||"(none)"}
                          </button>
                        );})}
                      </div>
                  ...
                  {/* Item count */}
                  <div style={{padding:"4px 8px",fontSize:9,color:"#5a6080"}}>{filtItems.length} item{filtItems.length!==1?"s":""}</div>
                </>
              )}
            </div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",background:"#0f1117"}}>
          ...
          {tab==="moves"&&selMove&&(
            <div style={{padding:"20px 24px"}}>
              ...
              {(()=>{
                ...
                const RANK_COLORS2:Record<string,string>={Starter:"#78c850",Rookie:"#6890f0",Standard:"#f8d030",Advanced:"#f08030",Expert:"#a040a0",Ace:"#e04040",Master:"#705898",Champion:"#ffd700"};
                return(
                  <div style={{marginTop:16}}>
                    <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:8}}>Learned by ({learners.length} Pokémon)</div>
                    <div style={{maxHeight:200,overflowY:"auto",display:"flex",flexWrap:"wrap",gap:4}}>
                      {learners.map(p=>{
                        ...
                        return(
                          <div key={`${p.number}-${p.name}`} style={{display:"flex",alignItems:"center",gap:4,background:"#13151f",borderRadius:4,padding:"3px 8px",cursor:"pointer"}}
                            onClick={()=>{/* Could select pokemon */}}>
                            <span style={{fontSize:9,color:"#3a4060",fontFamily:"'Exo 2'",fontWeight:700}}>#{String(p.number).padStart(3,"0")}</span>
                            <span style={{fontSize:11,color:"#e8eaf0"}}>{p.name}</span>
          ...
          {tab==="items"&&(()=>{
            ...
            if(filtItems.length===0)return(
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",color:"#5a6080",flexDirection:"column",gap:8}}>
            ...
            return(
              <div style={{padding:"16px 20px"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,flexWrap:"wrap"}}>
                  <h2 style={{fontFamily:"'Exo 2'",fontWeight:800,fontSize:20,color:"#e8eaf0",margin:0}}>
                ...
                {sortedPockets.map(pocket=>{
                  const pocketCfg=POCKET_LABELS[pocket]||{label:pocket,color:"#5a6080",icon:"📦"};
                  ...
                  return(
                    <div key={pocket} style={{marginBottom:24}}>
                      ...
                      {cats.map(cat=>{
                        const catColor=CAT_ITEM_COLORS[cat==="General"?"":cat]||"#5a6080";
                          ...
                          <div key={cat} style={{marginBottom:16}}>
                            {(cats.length>1||itemPocketFilter)&&cat!=="General"&&(
                              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                                ...
                                <span style={{fontSize:9,color:"#5a6080"}}>{items.length}</span>
                              </div>
                            )}
                            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:8}}>
                              {items.map(item=>{
                                const catCol=CAT_ITEM_COLORS[item.category]||"#5a6080";
                                return(
                                  <div key={item.name} style={{background:"#1e2235",border:"1px solid #2a2f45",borderRadius:7,padding:"10px 12px",borderLeft:`3px solid ${catCol}60`}}>
                                    <div style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:6}}>
                                      <div style={{flex:1}}>
                                        <div style={{fontSize:13,fontWeight:700,color:"#e8eaf0",lineHeight:1.3}}>{item.name}</div>
                                      ...
                                      <div style={{textAlign:"right",flexShrink:0}}>
                                        {item.cost&&item.cost!=="Not for Sale"&&<div style={{fontSize:11,fontWeight:700,color:"#ffd32a",fontFamily:"'Exo 2'"}}>{item.cost}₽</div>}
                                        {item.cost==="Not for Sale"&&<div style={{fontSize:9,color:"#5a6080"}}>Not for sale</div>}
                                    ...
                                    <p style={{fontSize:11,color:"#8b90a8",lineHeight:1.5,margin:"0 0 8px"}}>{item.description}</p>
                                    {item.forPokemon&&<div style={{fontSize:10,color:"#00d4aa",marginBottom:6}}>For: {item.forPokemon}</div>}
                                    <button onClick={()=>setAddModalItem(item)}
                                      style={{fontSize:10,padding:"4px 10px",borderRadius:4,cursor:"pointer",fontWeight:700,background:"rgba(255,211,42,0.08)",border:"1px solid #ffd32a30",color:"#ffd32a",width:"100%"}}>
                                      🎒 Add to Inventory
                                    </button>
                                  </div>
                                );
