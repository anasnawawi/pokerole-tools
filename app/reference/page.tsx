            ...
            <div style={{flex:1,overflowY:"auto",padding:4}}>
              ...
              {tab==="items"&&filtItems.map(i=>(
                <div key={i.name} onClick={()=>setSelItem(i)} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",borderRadius:4,cursor:"pointer",background:selItem?.name===i.name?"#242842":"transparent",borderLeft:`2px solid ${selItem?.name===i.name?"#ffd32a":"transparent"}`}}
                  ...
                  <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#e8eaf0"}}>{i.name}</div><div style={{fontSize:10,color:"#5a6080",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{i.category}{i.pocket?" · "+i.pocket:""}</div></div>
                  {i.cost&&<span style={{fontSize:10,color:"#ffd32a"}}>{i.cost}</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",background:"#0f1117"}}>
          ...
          {tab==="items"&&selItem&&(
            <div style={{padding:"20px 24px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div style={{background:"#13151f",borderRadius:5,padding:"10px 12px"}}><div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:3}}>Category</div><div style={{fontSize:13,color:"#e8eaf0",fontWeight:600}}>{selItem.category||"—"}</div></div>
                <div style={{background:"#13151f",borderRadius:5,padding:"10px 12px"}}><div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:3}}>Pocket</div><div style={{fontSize:13,color:"#e8eaf0",fontWeight:600}}>{selItem.pocket||"—"}</div></div>
                <div style={{background:"#13151f",borderRadius:5,padding:"10px 12px"}}><div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:3}}>Cost</div><div style={{fontSize:13,color:"#ffd32a",fontWeight:600,fontFamily:"'Exo 2'"}}>{selItem.cost||"—"}</div></div>
                <div style={{background:"#13151f",borderRadius:5,padding:"10px 12px"}}><div style={{fontSize:9,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:3}}>One-Use</div><div style={{fontSize:13,color:"#e8eaf0",fontWeight:600}}>{selItem.oneUse?"Yes":"No"}</div></div>
              </div>
              {selItem.forPokemon&&<div style={{marginTop:10,background:"rgba(0,212,170,0.08)",border:"1px solid #00d4aa30",borderRadius:5,padding:"8px 12px",fontSize:12,color:"#00d4aa"}}>For Pokémon: {selItem.forPokemon}</div>}

              {/* Give to Trainer inventory */}
              {(()=>{
                const trainers=loadFromStorage<any[]>("trainers",[]);
                if(trainers.length===0)return<div style={{marginTop:12,fontSize:11,color:"#5a6080"}}>No characters saved. Create characters to give items to them.</div>;
                return(
                  <div style={{marginTop:14}}>
                    <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:8}}>🎒 Give to Trainer</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {trainers.map((t:any)=>(
                        <button key={t.id} onClick={()=>{
                          const saved=loadFromStorage<any[]>("trainers",[]);
                          const idx=saved.findIndex((x:any)=>x.id===t.id);
                          if(idx<0)return;
                          const inv=[...(saved[idx].inventory||[])];
                          const existing=inv.findIndex((i:any)=>i.name===selItem.name);
                          if(existing>=0)inv[existing].quantity=(inv[existing].quantity||1)+1;
                          else inv.push({name:selItem.name,quantity:1,description:selItem.description});
                          saved[idx]={...saved[idx],inventory:inv};
                          saveToStorage("trainers",saved);
                          alert(`Added ${selItem.name} to ${t.name}'s inventory!`);
                        }} style={{padding:"6px 12px",borderRadius:5,fontSize:11,fontWeight:700,cursor:"pointer",background:"rgba(255,211,42,0.1)",border:"1px solid #ffd32a40",color:"#ffd32a"}}>
                          + Give to {t.name}
                        </button>
                      ))}
                    </div>
