...
function MovePopup({move,attacker,allEntries,weather,onClose,onApplyDmg,onApplyEffect}:{
  ...
  if(el.includes("increase")&&(el.includes("defense")||el.includes("vitality")))statFx.push({attr:"vitality",amount:1});
  if(el.includes("increase")&&el.includes("strength"))statFx.push({attr:"strength",amount:1});
  if(el.includes("increase")&&el.includes("dexterity"))statFx.push({attr:"dexterity",amount:1});
  if(el.includes("increase")&&el.includes("special"))statFx.push({attr:"special",amount:1});
  if(el.includes("increase")&&el.includes("evasion"))statFx.push({attr:"dexterity",amount:1});
        ...
        <div style={{padding:14,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,flex:1}}>
          ...
          {canAct&&(
            <div>
              ...
              <div style={{fontSize:10,color:"#5a6080",marginBottom:6,fontStyle:"italic"}}>Pool: {accBreakdown} = <strong style={{color:"#6890f0"}}>{accPool}d</strong></div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <button onClick={()=>setAccResult(rollDice(accPool))} style={{background:"#6890f020",border:"1px solid #6890f060",borderRadius:4,color:"#6890f0",padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer"}}>🎲 Roll Accuracy ({accPool}d)</button>
          ...
          {/* Support move stat changes */}
          {canAct&&accResult&&accResult.successes>=actReq&&statFx.length>0&&targets.length>0&&(
            <div style={{background:"#13151f",borderRadius:6,padding:"10px 12px"}}>
              <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:8}}>Stat Changes</div>
              {statFx.map((se,i)=>targets.map(tid=>{
                const isSelf=tid===attacker.id;
                const t=isSelf?attacker:allEntries.find(e=>e.id===tid);
                return<button key={`${i}-${tid}`} onClick={()=>onApplyEffect(tid,se.attr,se.amount,move.name)} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 10px",borderRadius:4,cursor:"pointer",background:se.amount<0?"rgba(255,71,87,0.1)":"rgba(0,212,170,0.1)",border:`1px solid ${se.amount<0?"#ff475730":"#00d4aa30"}`,color:se.amount<0?"#ff4757":"#00d4aa",fontSize:11,fontWeight:700,width:"100%",marginBottom:3}}>
                  {se.amount>0?"▲":"▼"} Apply {se.attr} {se.amount>0?"+":"−"}1 to {isSelf?"SELF":t?.nickname||t?.pokemon.name}
                </button>;
              }))}
            </div>
          )}
...
// ── Capture Popup (full) ──────────────────────────────────────────────────────
function CapturePopup({allEntries,onClose}:{allEntries:BattleEntry[];onClose:()=>void;}){
  ...
  // Fallback: standard balls if no inventory
  const ballOptions=balls.length>0?balls:[
      ...
      <div style={{background:"#1e2235",border:"1px solid #3a4060",borderRadius:10,width:480,maxHeight:"88vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.8)"}}>
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {ballOptions.map(b=><button key={b.name} onClick={()=>setBallKey(b.name)} style={{flex:1,minWidth:80,padding:"7px 6px",borderRadius:5,border:`1px solid ${ballKey===b.name?"#ffd32a":"#3a4060"}`,background:ballKey===b.name?"rgba(255,211,42,0.15)":"#13151f",color:ballKey===b.name?"#ffd32a":"#8b90a8",fontSize:11,fontWeight:ballKey===b.name?700:400,cursor:"pointer",textAlign:"center"}}>
                {b.name}<div style={{fontSize:9,color:"#5a6080",marginTop:1}}>{getSealDice(b.name)}d seal</div>
              </button>)}
            </div>
          </div>
          ...
          {throwerId&&targetId&&ballKey&&(
            <div>
              <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:6}}>4. Throw Roll: SPC + Channel + 2 Skill = {throwPool}d</div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                {throwRoll&&<span style={{fontSize:12,fontFamily:"'Exo 2'",fontWeight:700}}>[{throwRoll.rolls.join(",")}] = {throwRoll.successes} hits</span>}
              </div>
            </div>
          )}
...
function BattleCard({entry,allEntries,weather,isActive,onUpdate,onRemove,onDragStart,onDragOver,onDrop}:{
  ...
  return(
    <>
      ...
      {showCapture&&<CapturePopup allEntries={allEntries} onClose={()=>setShowCapture(false)}/>}
      ...
      <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}
        ...
        <div style={{padding:"4px 8px",background:"#13151f",display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
          ...
          {linkedTrainer&&<button onClick={()=>setShowTrainerSkills(true)} style={{background:"rgba(61,139,255,0.15)",border:"1px solid #3d8bff30",borderRadius:3,color:"#3d8bff",cursor:"pointer",fontSize:9,padding:"1px 5px"}}>👤 Skills</button>}
        </div>
        ...
        <div style={{padding:"5px 8px",flex:1,display:"flex",flexDirection:"column",gap:3,minHeight:0}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:2}}>
            <span style={{fontSize:8,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase"}}>Moves</span>
          ...
          {showEditMoves?(
            <div style={{maxHeight:120,overflowY:"auto"}}>
              {MOVES.filter(m=>{
                ...
                return entry.pokemon.moves.some(pm=>pm.name.toLowerCase()===q)||
                       entry.moves.some(em=>em.name===m.name);
              }).concat(MOVES.filter(m=>!entry.pokemon.moves.some(pm=>pm.name===m.name)&&!entry.moves.some(em=>em.name===m.name)).slice(0,50))
              .slice(0,80).map(m=>{const has=entry.moves.some(em=>em.name===m.name);return(
                <div key={m.name} style={{display:"flex",alignItems:"center",gap:4,padding:"1px 0"}}>
            ...
            <>
              {entry.moves.map((m,i)=>{
                const stab=entry.pokemon.types.includes(m.type as PokemonType);
              ...
              {entry.moves.length===0&&<div style={{fontSize:9,color:"#5a6080",fontStyle:"italic"}}>No moves — click Edit</div>}
            </>
          )}
        </div>
  ...
  const nextTurn=()=>{
    if(activeEntry){
      setEntries(prev=>prev.map(e=>{
        ...
        return{...e,hasTakenTurn:true,actionCount:0,status:ns,statusTurnsLeft:nt};
      }));
    }
    ...
    if(next===0){
      ...
      setShowEOR(true);
      if(entries.some(e=>e.currentHp>0&&e.moves.some(m=>(m.priority??0)>0)))setTimeout(()=>setShowPriority(true),400);
    }
    setTurn(next);
  };
