      ...
      if(atkRoll.successes>dr.successes){
        ...
        if(onApplyEffect){
          ...
          if(el.includes("increase user")||el.includes("increase the user")||statAppliestoSelf(move)){
            if(el.includes("strength")&&el.includes("increase"))onApplyEffect(attacker.id,"strength",1,move.name);
            if(el.includes("special")&&el.includes("increase"))onApplyEffect(attacker.id,"special",1,move.name);
            if(el.includes("defense")&&el.includes("increase")&&!el.includes("sp."))onApplyEffect(attacker.id,"vitality",1,move.name);
          }
        }
        ...
        <div style={{padding:14,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,flex:1}}>
          ...
          {canAct&&(
            <div>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                {accResult&&<button onClick={()=>onIncrementAction(attacker.id,isPriority)} style={{padding:"3px 8px",background:"rgba(0,212,170,0.1)",border:"1px solid #00d4aa30",borderRadius:3,fontSize:9,color:"#00d4aa",cursor:"pointer"}}>+Action</button>}
              </div>
            </div>
          )}
          ...
          {canAct&&(accResult?.successes??0)>=actReq&&(
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              ...
              {/* Status inflict (Poison Gas, Will-O-Wisp, etc.) */}
              {statusToInflict&&targets.length>0&&(
                <div style={{background:"#13151f",borderRadius:6,padding:"10px 12px"}}>
                  <div style={{fontSize:10,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:6}}>Apply Status</div>
                  {targets.filter(t=>t!==attacker.id).map(tid=>{
                    const t=allEntries.find(e=>e.id===tid);if(!t)return null;
                    return<button key={tid} onClick={()=>{if(onApplySpecial)onApplySpecial(tid,{status:statusToInflict!,statusTurnsLeft:statusToInflict==="Asleep"?3:0});onIncrementAction(attacker.id,isPriority);}} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 10px",borderRadius:4,cursor:"pointer",background:"rgba(160,64,160,0.1)",border:"1px solid #a040a040",color:"#a040a0",fontSize:11,fontWeight:700,width:"100%",marginBottom:3}}>
              ...
              {isTransformMove&&onApplySpecial&&(
                <div style={{background:"rgba(160,64,160,0.08)",border:"1px solid #a040a040",borderRadius:6,padding:"10px 12px"}}>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {others.map(t=><button key={t.id} onClick={()=>{onApplySpecial!(attacker.id,{morphedTo:t.pokemon,moves:t.moves.slice(0,4),attrs:{...t.attrs}});onIncrementAction(attacker.id,false);onClose();}} style={{padding:"5px 10px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",background:"rgba(160,64,160,0.15)",border:"1px solid #a040a040",color:"#e8eaf0"}}>
              ...
              <button onClick={()=>{onIncrementAction(attacker.id,isPriority);}} style={{padding:"5px 10px",background:"rgba(0,212,170,0.1)",border:"1px solid #00d4aa30",borderRadius:4,fontSize:10,color:"#00d4aa",cursor:"pointer"}}>
                ✓ Mark Action Used ({isPriority?"Reaction":"Action #{"+String((attacker.actionCount||0)+1)+"}"})
              </button>
            </div>
          )}
...
function PriorityPopup({entries,allEntries,weather,onClose,onApplyDmg,onApplyEffect,onIncrementAction,onSpendWP,onApplySpecial}:{
  ...
  return(
    <>
    {activeMove&&<MovePopup move={activeMove.move} attacker={activeMove.entry} allEntries={allEntries} weather={weather} onClose={()=>setActiveMove(null)} onApplyDmg={onApplyDmg} onApplyEffect={onApplyEffect} onIncrementAction={onIncrementAction} onSpendWP={onSpendWP} onApplySpecial={onApplySpecial}/>}
    ...
    <>
      ...
      <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}
        ...
        <div style={{padding:"6px 8px",background:isActive?sideColor+"18":"#0f1117",borderRadius:"6px 6px 0 0",display:"flex",alignItems:"center",gap:5}}>
          ...
          {linkedTrainer&&<button onClick={()=>upd({showTrainerView:!entry.showTrainerView})} style={{background:entry.showTrainerView?"rgba(61,139,255,0.3)":"rgba(61,139,255,0.1)",border:`1px solid ${entry.showTrainerView?"#3d8bff":"#3d8bff30"}`,borderRadius:3,color:"#3d8bff",cursor:"pointer",fontSize:8,padding:"1px 4px",flexShrink:0}} title="Toggle Trainer/Pokémon view">{entry.showTrainerView?"🐾":"👤"}</button>}
          ...
          <div style={{borderTop:"1px solid #2a2f45",padding:"8px 8px",display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div>
              ...
              {entry.statMods.length>0&&<div style={{marginTop:4,display:"flex",flexWrap:"wrap",gap:2}}>
                {entry.statMods.map((m,i)=><div key={i} title={`Source: ${m.source} | ${m.attr} ${m.amount>0?"+":""}${m.amount}`} style={{fontSize:8,display:"flex",alignItems:"center",gap:2,background:m.amount>0?"rgba(0,212,170,0.1)":"rgba(255,71,87,0.1)",border:`1px solid ${m.amount>0?"#00d4aa30":"#ff475730"}`,borderRadius:2,padding:"0 4px",cursor:"help"}}>
            ...
            {entry.side==="player"&&linkedTrainer&&(linkedTrainer.inventory||[]).length>0&&(
              <div>
                <div style={{fontSize:8,color:"#5a6080",letterSpacing:"1px",textTransform:"uppercase",marginBottom:4}}>🎒 Items ({linkedTrainer.name})</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                  {(linkedTrainer.inventory as {name:string;quantity:number}[]).map((item,i)=>(
                    <button key={i} onClick={()=>{
                      const hp=Math.min(entry.maxHp,entry.currentHp+(item.name.toLowerCase().includes("potion")?10:item.name.toLowerCase().includes("super")? 20:item.name.toLowerCase().includes("hyper")?50:item.name.toLowerCase().includes("max")?entry.maxHp:5));
                      const isHealItem=item.name.toLowerCase().includes("potion")||item.name.toLowerCase().includes("berry")||item.name.toLowerCase().includes("revive");
                      if(isHealItem){upd({currentHp:hp});}
                      alert(`Used ${item.name} on ${entry.nickname||entry.pokemon.name}!`);
                    }} style={{padding:"2px 7px",borderRadius:3,fontSize:9,cursor:"pointer",background:"rgba(255,211,42,0.1)",border:"1px solid #ffd32a30",color:"#ffd32a"}}>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#0f1117",color:"#e8eaf0",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div ref={scrollRef} style={{flex:1,overflowX:"auto",overflowY:"hidden",padding:"10px 10px"}}
