...
function PopoutButton({panelType,panelLabel}:{panelType:PanelType;panelLabel:string}) {
  const openPopout=()=>{
    ...
    if(panelType==="quick_roll"){
      ...
      w.document.write(`<!DOCTYPE html><html><head><title>Quick Roller</title><style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0f1117;color:#e8eaf0;font-family:Inter,sans-serif;padding:16px}.row{display:flex;gap:8px;align-items:center;margin-bottom:8px}button{background:#1e2235;border:1px solid #3a4060;border-radius:4px;color:#6890f0;padding:4px 14px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit}span{font-size:12px;color:#e8eaf0;font-family:'Exo 2',sans-serif}h2{font-family:'Exo 2',sans-serif;margin-bottom:12px;font-size:16px;color:#6890f0}</style></head><body><h2>🎲 Quick Roller</h2>${rows}<script>function roll(n){const rolls=Array.from({length:n},()=>Math.floor(Math.random()*6)+1);const succ=rolls.filter(r=>r>=4).length;document.getElementById('r'+n).textContent='['+rolls.join(',')+'] = '+succ+'✓';}</script></body></html>`);
      w.document.close();return;
    }
    // Reference panels — write static content
    const styles=`*{box-sizing:border-box;margin:0;padding:0}body{background:#0f1117;color:#e8eaf0;font-family:Inter,sans-serif;padding:12px;font-size:12px}h2{font-family:'Exo 2',sans-serif;font-size:16px;margin-bottom:10px}h3{font-family:'Exo 2',sans-serif;font-size:12px;color:#5a6080;text-transform:uppercase;letter-spacing:1px;margin:12px 0 5px}.card{background:#1e2235;border-radius:5px;padding:9px 11px;margin-bottom:7px}.row{display:flex;justify-content:space-between;padding:2px 0}`;
    ...
    if(panelType==="status_ref"){body+=Object.entries(SC).map(([n,s])=>`<div class="card" style="border-left:3px solid ${s.color}"><div style="font-weight:700;color:${s.color};margin-bottom:3px">${n}</div><div style="font-size:11px;color:#8b90a8">${s.desc}</div></div>`).join("");}
    else if(panelType==="weather_ref"){body+=`<div class="card"><b style="color:#ffd32a">☀️ Sunny</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Fire +2 dice, Water −2 dice. Solar Beam instant.</p></div><div class="card"><b style="color:#6890f0">🌧️ Rain</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Water +2 dice, Fire −2 dice. Thunder always hits.</p></div><div class="card"><b style="color:#e0c068">🌪️ Sandstorm</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Non-Rock/Ground/Steel: −1 HP/round.</p></div><div class="card"><b style="color:#98d8d8">🧊 Hail</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Non-Ice: −1 HP/round. Blizzard always hits.</p></div><div class="card"><b style="color:#8b90a8">🌫️ Fog</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">All accuracy rolls −1 die.</p></div><div class="card"><b style="color:#f8d030">⚡ Electric Terrain</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Electric +1 die. Grounded cannot sleep.</p></div><div class="card"><b style="color:#EE99AC">🌸 Misty Terrain</b><p style="font-size:11px;color:#8b90a8;margin-top:4px">Fairy +1 die. Grounded cannot be statused.</p></div>`;}
    else if(panelType==="catch_ref"){body+=`<h3>Required Successes</h3>${[["Starter","3"],["Rookie","4"],["Standard","6"],["Advanced","8"],["Expert","9"],["Ace","10"],["Master","12"],["Champion","14"]].map(([r,s])=>`<div class="row"><span>${r}</span><strong style="color:#ffd32a">${s}</strong></div>`).join("")}<h3>Bonuses</h3>${[["Half HP","+1"],["1 HP","+2"],["Status Ailment","+1 each"]].map(([c,v])=>`<div class="row"><span style="color:#8b90a8">${c}</span><span style="color:#00d4aa">${v}</span></div>`).join("")}<h3>Ball Seal Potency</h3>${[["Pokéball","4d"],["Great Ball","6d"],["Ultra Ball","8d"]].map(([b,p])=>`<div class="row"><span style="color:#8b90a8">${b}</span><span style="color:#ffd32a">${p}</span></div>`).join("")}`;}
    else if(panelType==="rules"){body+=`${[["🎲 Roll","Attribute + Skill. Each 4/5/6 = 1 success."],["⚡ Actions","1st=1 needed, 2nd=2, 3rd=3, 4th=4, 5th=5."],["💥 Physical Dmg","STR + Power − foe VIT. Min 1 die, min 1 dmg."],["💫 Special Dmg","SPC + Power − foe INS. Min 1 die, min 1 dmg."],["⭐ STAB","+1 die when move type matches Pokémon type."],["🔴 Super Effective","+2 dice to damage pool (needs 1+ succ)."],["💢 Critical Hit","3+ extra hits → +2 dmg dice."],["🩹 Pain (>50%)","No pen. 26–50%: −1d. 1–25%: −2d. 0: Fainted."],["⚠ Disobedience","Same/lower: obeys. +1 rank: loyalty roll (3+). +2: ignores."]].map(([t,d])=>`<div class="card"><div style="font-weight:700;margin-bottom:3px">${t}</div><div style="font-size:11px;color:#8b90a8">${d}</div></div>`).join("")}`;}
    else{body+=`<p style="color:#5a6080;margin-top:8px">This panel (${panelLabel}) is for reference. Open it in the GM Screen for interactive features.</p>`;}
    w.document.write(`<!DOCTYPE html><html><head><title>${panelLabel} — PokeRole</title><style>${styles}</style></head><body>${body}</body></html>`);
    w.document.close();
  };
  return <button onClick={openPopout} title="Pop out" style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:12,padding:"0 4px"}}>↗</button>;
}
...
function TrackerOverviewPanel({entries,setEntries}:{entries:BattleEntry[];setEntries:React.Dispatch<React.SetStateAction<BattleEntry[]>>}){
  ...
  const sideColor=active?{player:"#00d4aa",enemy:"#ff4757",neutral:"#8b90a8"}[active.side]:"#5a6080";
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",fontSize:11}}>
      {/* Header */}
      <div style={{padding:"5px 8px",background:"#0f1117",borderBottom:"1px solid #2a2f45",display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
        ...
        <button onClick={nextTurn} style={{background:"#00d4aa",color:"#0f1117",border:"none",borderRadius:3,padding:"2px 8px",fontWeight:700,fontSize:10,cursor:"pointer"}}>Next ▶</button>
        <a href="/battle-tracker" target="_blank" style={{background:"rgba(61,139,255,0.15)",border:"1px solid #3d8bff30",borderRadius:3,color:"#3d8bff",padding:"2px 7px",fontSize:9,textDecoration:"none",whiteSpace:"nowrap"}}>Open Full ↗</a>
      </div>
      {/* Combatant list */}
      <div style={{flex:1,overflowY:"auto",padding:"4px"}}>
        {sorted.length===0&&<div style={{color:"#5a6080",padding:"16px 8px",textAlign:"center",fontSize:10}}>No combatants. Use the Battle Tracker (↗) to add Pokémon — this panel syncs automatically.</div>}
        {sorted.map((e,i)=>{
          const sc={player:"#00d4aa",enemy:"#ff4757",neutral:"#8b90a8"}[e.side];
          ...
          return(
            <div key={e.id} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 5px",borderRadius:4,background:active?.id===e.id?sc+"12":"transparent",borderLeft:`2px solid ${active?.id===e.id?sc:"transparent"}`,marginBottom:2,opacity:e.currentHp<=0?0.4:1}}>
              <span style={{fontSize:8,color:"#3a4060",width:14,textAlign:"right",flexShrink:0}}>{i+1}</span>
              <div style={{width:5,height:5,borderRadius:"50%",background:e.currentHp<=0?"#3a3040":TYPE_COLORS[e.pokemon.types[0]],flexShrink:0}}/>
              <span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:11,color:e.currentHp<=0?"#5a6080":"#e8eaf0",textDecoration:e.currentHp<=0?"line-through":"none"}}>{e.nickname||e.pokemon.name}</span>
              ...
              {e.hasTakenTurn&&<span style={{fontSize:7,color:"#3a4060",flexShrink:0}}>✓</span>}
              <span style={{fontSize:9,color:e.currentHp<=0?"#705898":e.currentHp/e.maxHp>0.5?"#00d4aa":e.currentHp/e.maxHp>0.25?"#ffd32a":"#ff4757",fontFamily:"'Exo 2'",fontWeight:700,flexShrink:0}}>{e.currentHp}/{e.maxHp}</span>
            </div>
          );
        })}
      </div>
...
export default function GMScreen() {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#0f1117",overflow:"hidden"}}>
      ...
      {/* 4x3 Panel Grid — colSpan/rowSpan merging via → ← ↓ ↑ buttons */}
      <div style={{flex:1,display:"grid",gridTemplateColumns:"repeat(4,1fr)",gridTemplateRows:"repeat(3,1fr)",gap:2,padding:2,overflow:"auto",background:"#0a0c14"}}>
        {grid.map((panel,i)=>{
          const col=i%4;const row=Math.floor(i/4);
          // Skip slots covered by a spanning panel
          const isCovered=grid.some((p,j)=>{
            if(!p||j===i)return false;
            const pc=j%4;const pr=Math.floor(j/4);
          ...
          const setSpan=(cs:number,rs:number)=>{const g=[...grid];if(panel)g[i]={...panel,colSpan:Math.max(1,Math.min(4-col,cs)),rowSpan:Math.max(1,Math.min(3-row,rs))};setGrid(g);};
          return(
            <div key={i} style={{background:"#13151f",border:"1px solid #1a1d27",borderRadius:4,display:"flex",flexDirection:"column",overflow:"hidden",gridColumn:`${col+1}/span ${colSpan}`,gridRow:`${row+1}/span ${rowSpan}`}}>
              {panel===null?(
                ...
                <>
                  <div style={{display:"flex",alignItems:"center",gap:3,padding:"4px 8px",background:"#0f1117",borderBottom:"1px solid #1a1d27",flexShrink:0}}>
                    <span style={{fontSize:12,flexShrink:0}}>{PANEL_CATALOG.find(p=>p.type===panel.type)?.icon}</span>
                    <span style={{fontSize:11,fontWeight:600,color:"#8b90a8",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{PANEL_CATALOG.find(p=>p.type===panel.type)?.label}</span>
                    <span style={{fontSize:8,color:"#3a4060",flexShrink:0,padding:"0 2px"}}>{colSpan}×{rowSpan}</span>
                    {col+colSpan<4&&<button onClick={()=>setSpan(colSpan+1,rowSpan)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:10,padding:"0 1px"}} title="Expand right">→</button>}
                    {colSpan>1&&<button onClick={()=>setSpan(colSpan-1,rowSpan)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:10,padding:"0 1px"}} title="Shrink right">←</button>}
                    {row+rowSpan<3&&<button onClick={()=>setSpan(colSpan,rowSpan+1)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:10,padding:"0 1px"}} title="Expand down">↓</button>}
                    {rowSpan>1&&<button onClick={()=>setSpan(colSpan,rowSpan-1)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:10,padding:"0 1px"}} title="Shrink up">↑</button>}
                    <PopoutButton panelType={panel.type} panelLabel={PANEL_CATALOG.find(p=>p.type===panel.type)?.label||"Panel"}/>
                    <button onClick={()=>setPickerSlot(i)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:11,padding:"0 2px"}} title="Change">⇄</button>
                    <button onClick={()=>clearPanel(i)} style={{background:"none",border:"none",color:"#5a6080",cursor:"pointer",fontSize:12}} title="Remove">✕</button>
                  </div>
